...
const index: FC<Props> = ({ vacancy }) => {
  const { id, token, isLoading, error } = useSelector(
    (store: IState) => store.signIn
  );

  // const { vacancies } = useSelector((store: IState) => store.vacancies);
  ...
  const item = Array(45)
    .fill("")
    .map((_, idx) =>
      idx >= (active - 1) * 3 && idx <= (active - 1) * 3 + 2 ? (
        <Vacancy opened={opened} auth={id} key={idx} id={idx} shadow />
      ) : null
    );

  return (
    <BrandPage className={styles.container}>
      ...
      {vacancy != true ? (
        <>
                ...
                {id && (
                  <Col lg={{ offset: 2, span: 3 }}>
                    <BtnPrimary
                      ...
                      blue
                      className="px-0"
                      onClick={() => route.push("/companies/publishVacancy")}
                    />
                  </Col>
                )}
              </Row>
          ...
          <Row className={styles.pagination}>
            <Col lg={{ offset: 0, span: 2 }} xs={{ offset: 3, span: 6 }}>
              {" "}
              <BtnPrimary title="Load more" blue />{" "}
            </Col>
